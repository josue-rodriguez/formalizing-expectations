...
sim_constraints <- read.csv("data/02-sim-constraints.csv")
sim_sd <- read.csv("data/03-sim-sd.csv")
...
constraints_plot_data <- 
  tidy_constraints %>% 
  as_tibble %>% 
  pivot_longer(h12:h13,
               names_to = "hypothesis",
               values_to = "pmp") %>% 
  group_by(sd, const, hypothesis, n) %>% 
  summarise(mean_pmp = mean(pmp)) %>% 
  ungroup %>% 
  mutate(hypothesis = recode(hypothesis, h12 = "H2", h13 = "H3"),
         const = factor(const),
